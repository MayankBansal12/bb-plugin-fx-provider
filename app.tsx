...
/**
 * The official fx mark.
 *
 * The same artwork ships as `./assets/fx.svg` for the manifest branding and the
 * provider declaration, but a path-shaped provider icon is drawn through
 * `<img>` — a separate document, where `currentColor` resolves to black and
 * disappears on dark themes. Registering the mark as a component renders it
 * inline instead, so it inherits the host's provider color class.
 */
...
export default definePluginApp((app) => {
  app.slots.experimental_providerIcon({ providerId: "fx", icon: FxIcon });
});
